...
public class Tree implements GraphicalObject
{
	...
	public Tree( float x, float y )
	{
		col = new Color((float) Math.random() / 1.0f,
				(float) Math.random() / 1.0f, (float) Math.random() / 1.0f);
		...
		buildTree(x, y, 0.0f, length, 1, 1, 1, 0);
	...
	{
		return ( factor < 1 ) ? 1 : leftFactorial(factor - 1)
				+ factorial(factor);
	}

	public int buildTree( float x, float y, float z, double dist,
			int subBranches, int index, int arrayIndex, int previousArrIndex )
	{
		double radians = ( 2 * Math.PI )
				* ( (double) index / (double) subBranches ); // equal spaced
																// branching
																// horizontally
		double spacing = (double) ( ( 1 + numBranches ) - subBranches )
				* ( branchAngle - ( (double) subBranches * 0.1d ) ); // changing
																		// vertical
																		// branching
			...
			for ( int i = 1; i < ( subBranches + 2 ); ++i )
			{
				arrayIndex = buildTree(
						...
						( Math.abs(Math.sin(vertices.get(actualIndex))
								...
								+ ( dist * shrinkage ), subBranches + 1, i,
	...
	public boolean draw( GL2 gl )
	{
		...
		gl.glDrawElements(GL.GL_LINES, indices.capacity(), GL.GL_UNSIGNED_INT,
				indices);
		return true;
	}
